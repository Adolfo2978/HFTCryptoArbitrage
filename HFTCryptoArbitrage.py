...
class ArbitrageApp:
    ...
    def _apply_theme(self):
        ...
        table_row = "#1e293b" if dark else "#eef2ff"
        input_bg = "#0b1220" if dark else "#ffffff"
        info = "#60a5fa" if dark else "#1d4ed8"
        warn = "#f59e0b" if dark else "#b45309"
        error = "#f87171" if dark else "#b91c1c"
        ...
        self.style.configure("TNotebook.Tab", padding=(18, 10), font=("Segoe UI", 11, "bold"))
        self.style.configure("TButton", font=("Segoe UI", 10, "bold"), padding=7)
        self.style.configure("TLabelframe", background=bg, foreground=fg)
        self.style.configure("TLabelframe.Label", background=bg, foreground=fg, font=("Segoe UI", 11, "bold"))
        ...
        self.style.configure("CardTitle.TLabel", background=card, foreground=muted, font=("Segoe UI", 10, "bold"))
        self.style.configure("CardValue.TLabel", background=card, foreground="#34d399" if dark else "#047857", font=("Segoe UI", 16, "bold"))
        self.style.configure("Status.TLabel", background=bg, foreground=accent, font=("Segoe UI", 11, "bold"))
        ...
        self.style.configure(
            ...
            rowheight=30,
            font=("Segoe UI", 10),
            bordercolor="#334155" if dark else "#d1d5db",
        )
        self.style.configure(
            "Treeview.Heading",
            background="#334155" if dark else "#e5e7eb",
        ...
        self.style.map("Treeview.Heading", background=[("active", "#475569" if dark else "#d1d5db")])

        if hasattr(self, "tree"):
            self.tree.tag_configure("profit", foreground="#34d399" if dark else "#047857")
            self.tree.tag_configure("loss", foreground="#f87171" if dark else "#b91c1c")
            self.tree.tag_configure("best", background=table_row)
        ...
        if hasattr(self, "execution_text"):
            ...
            self.execution_text.tag_configure("INFO", foreground=info)
            self.execution_text.tag_configure("WARN", foreground=warn)
            self.execution_text.tag_configure("ERROR", foreground=error)
    ...
    def _build_ui(self):
        ...
        left.pack(side="left")
        ttk.Label(left, text="🚀 HFTCryptoArbitrage", font=("Segoe UI", 16, "bold")).pack(anchor="w")
        ttk.Label(left, text="Sistema unificado de arbitraje Binance", font=("Segoe UI", 11)).pack(anchor="w")
    ...
    def _build_statusbar(self):
        ...
        ttk.Label(status, text="v1.2.0 • Python 3.10+", font=("Segoe UI", 9), foreground=self.theme["neutral"]).pack(side="right")
    ...
    def _build_tab_stats(self):
        ...
        ttk.Label(f, text="Historial de escaneos", font=("Segoe UI", 12, "bold")).grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(f, text="Detalle y gráfico ASCII", font=("Segoe UI", 12, "bold")).grid(row=3, column=0, sticky="w", pady=(6, 2))
        self.detail_text = tk.Text(f, height=12, wrap="word", font=("Consolas", 11))
    ...
    def _build_tab_exec(self):
        ...
        self.execution_text = tk.Text(f, height=10, wrap="word", font=("Consolas", 11))
        ...
        ttk.Label(f, text="Operaciones ejecutadas", font=("Segoe UI", 11, "bold")).grid(row=9, column=0, columnspan=2, sticky="w")
